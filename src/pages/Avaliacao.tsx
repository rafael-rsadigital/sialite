    ...
    return (
      <main className="min-h-screen paper-screen px-4 py-12">
        <div className="mx-auto max-w-md">
          ...
          <section className="review-card border border-slate-100/80 p-8 sm:p-10"><div className="mb-10 flex justify-center"><StarRating value={nota} onChange={setNota} /></div><div className="mb-6 space-y-5"><div><label className="text-xs font-bold uppercase tracking-[.1em] text-[#31495f]" htmlFor="nome-cliente">Como podemos te chamar</label><Input id="nome-cliente" value={nomeCliente} onChange={(e) => setNomeCliente(e.target.value)} placeholder="Seu nome" className="legacy-field mt-2 px-3 text-sm" /></div><div><label className="text-xs font-bold uppercase tracking-[.1em] text-[#31495f]" htmlFor="produto-servico">Qual o nome do produto adquirido ou do serviço realizado</label><Input id="produto-servico" value={produtoServico} onChange={(e) => setProdutoServico(e.target.value)} placeholder="Ex.: corte de cabelo, troca de óleo..." className="legacy-field mt-2 px-3 text-sm" /></div></div><div className="mb-8"><Textarea placeholder="Conte-nos sobre sua experiência (opcional)" value={comentario} onChange={(e) => setComentario(e.target.value)} className="min-h-[120px] resize-none rounded-lg border-slate-200 bg-slate-50 text-sm text-slate-700 placeholder:text-slate-400 focus:border-slate-300 focus:bg-white focus:ring-slate-100" /></div><Button onClick={handleRegistrar} className="legacy-button h-12 w-full text-sm font-bold" size="lg">Enviar avaliação</Button></section>
