...
export default function Avaliacao() {
  ...
  useEffect(() => {
    if (!submitted || !isPositive) return;
    setBlocosVisiveis(1);
    const timers = [
          ...
          {isPositive ? (
            <>
              <h1 className="display-title mt-7 min-h-[3.5rem] text-3xl text-[#f5f0e5]">
                {blocosVisiveis >= 1 && <span className="animate-fade-in inline-flex items-center gap-2">Que bom que gostou do nosso atendimento <span aria-hidden="true">😄</span></span>}
              </h1>
              <div className="mx-auto mt-5 min-h-[6.5rem] max-w-md space-y-3 text-sm leading-6 text-[#c1c9cf]">
                {blocosVisiveis >= 2 && <p className="animate-fade-in">Aguarde a nossa página no Google abrir para finalizar sua avaliação!</p>}
                {blocosVisiveis >= 3 && <p className="animate-fade-in">Se puder, deixe um comentário sobre o serviço e atendimento, isso ajuda outras pessoas a encontrarem a {empresa.nome_exibicao}.</p>}
      ...
      <main className="min-h-screen paper-screen px-4 py-10 sm:py-14">
        <div className="mx-auto max-w-md animate-fade-in">
          ...
          <section className="review-card border border-slate-100/80 p-7 sm:p-9">
            <div className="space-y-5"><div><label className="text-xs font-bold uppercase tracking-[.1em] text-[#31495f]" htmlFor="comentario-retorno">Seu relato</label><Textarea id="comentario-retorno" maxLength={2000} placeholder="Descreva o que podemos melhorar..." value={comentario} onChange={(e) => setComentario(e.target.value)} className="mt-2 min-h-[110px] resize-none rounded-lg border-slate-200 bg-slate-50 text-sm text-slate-700 placeholder:text-slate-400 focus:border-slate-300 focus:bg-white focus:ring-slate-100" /></div><div className="grid gap-4 sm:grid-cols-2"><div><label className="text-xs font-bold uppercase tracking-[.1em] text-[#31495f]" htmlFor="nome-retorno">Nome</label><Input id="nome-retorno" maxLength={120} value={nomeRetorno} onChange={(e) => setNomeRetorno(e.target.value)} placeholder="Como podemos chamar você?" className="legacy-field mt-2 px-3 text-sm" /></div><div><label className="text-xs font-bold uppercase tracking-[.1em] text-[#31495f]" htmlFor="telefone-retorno">Celular</label><Input id="telefone-retorno" maxLength={40} type="tel" value={telefoneRetorno} onChange={(e) => setTelefoneRetorno(e.target.value)} placeholder="(00) 00000-0000" className="legacy-field mt-2 px-3 text-sm" /></div><div className="sm:col-span-2"><label className="text-xs font-bold uppercase tracking-[.1em] text-[#31495f]" htmlFor="email-retorno">E-mail</label><Input id="email-retorno" maxLength={200} type="email" value={emailRetorno} onChange={(e) => setEmailRetorno(e.target.value)} placeholder="seu@email.com" className="legacy-field mt-2 px-3 text-sm" /><p className="mt-2 text-xs text-slate-500">Informe pelo menos um contato: celular ou e-mail.</p></div></div></div>
            <div className="mt-7 border-t border-[#d6cebf] pt-6"><Button onClick={handleFinalizar} disabled={enviando} className="legacy-button h-12 w-full gap-2 text-sm font-bold" size="lg">{enviando ? <><Loader2 className="h-4 w-4 animate-spin" />Aguarde...</> : "Registrar e finalizar"}</Button></div>
            ...
            <div className="mt-6 border-t border-[#d6cebf] pt-6"><button onClick={() => setEtapa("coleta")} className="flex w-full items-center justify-center gap-1.5 text-sm text-slate-500 transition-colors hover:text-slate-700"><Pencil className="h-3.5 w-3.5" strokeWidth={1.5} />Alterar nota</button></div>
          </section>
